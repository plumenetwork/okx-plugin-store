...
- If the reported version is **≥ `0.1.16`** (this skill's `metadata.version`): binary is current, skip to step 2.
...
Run `plugin-store --version`. If the CLI version is **newer** than this skill's `metadata.version` (`0.1.12`), display a one-time notice:
...
Parse the output and present it to the user as a clean table (name, category, description). The registry updates dynamically; this is the only source of truth.
