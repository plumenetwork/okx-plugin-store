...
impl SkillInstaller {
    ...
    pub async fn download_from_github(repo: &str, path: &str) -> Result<String> {
        let url = format!(
    ...
    async fn fetch_repo_tree(repo: &str) -> Result<Vec<String>> {
        let url = format!(
    ...
    pub async fn install_from_dir(
        ...
        let all_paths = Self::fetch_repo_tree(repo).await?;
        ...
        for file_path in &files {
            let content = Self::download_from_github(repo, file_path).await?;
    ...
    pub async fn discover_skills(repo: &str) -> Result<Vec<DiscoveredSkill>> {
        let all_paths = Self::fetch_repo_tree(repo).await?;
    ...
    pub async fn discover_all(repo: &str) -> Result<DiscoverResult> {
        let all_paths = Self::fetch_repo_tree(repo).await?;

        let skills = Self::extract_skills(&all_paths, repo);
        let mcps = Self::extract_mcps(repo, &all_paths).await?;
    ...
    async fn extract_mcps(repo: &str, all_paths: &[String]) -> Result<Vec<DiscoveredMcp>> {
        ...
        for mcp_path in mcp_files {
            let content = Self::download_from_github(repo, mcp_path).await?;
