        ...
        if !mcp_only {
            if let Some(ref skill) = plugin.components.skill {
                if let Some(ref dir) = skill.dir {
                    ...
                    let count = SkillInstaller::install_from_dir(&skill.repo, dir, &skill_dir).await?;
                    ...
                    let content = SkillInstaller::download_from_github(&skill.repo, path).await?;
                    let skill_dir = adapter.skill_dir(&plugin.name);
                    ...
                    let result = SkillInstaller::discover_all(&skill.repo).await?;
